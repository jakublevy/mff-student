...
begin
	select 
		h.Reg_Id AS 'Reg. č.', k.Jméno, k.Příjmení, dbo.Ml_Kategorie_Formátované(dbo.Urči_Ml_Kategorii(h.Reg_Id), h.Muž) as 'Hráčská kat.', k.Email, dbo.Tel_Číslo(k.Tel_Id) as 'Tel. číslo', 
...
begin
	...
	select Reg_Id AS 'Reg. č.', Jméno, Příjmení, dbo.Ml_Kategorie_Formátované(@kategorie, @p_b) as 'Hráčská kat.', Email, dbo.Tel_Číslo(Tel_Id) as 'Tel. číslo' from Hráč
...
create procedure dbo.Přidej_Adresu(@ulice nvarchar(30), @c_p nvarchar(10), @mesto nvarchar(30), @psc nvarchar(5), @adresa_id int output)
as
begin
...
create   procedure dbo.Přidej_Hostování(@kontrakt_id nvarchar(15), @od date, @do date, @cena_za_sezonu money, @hrac_reg_id nvarchar(7), @clenstvi_id int, @smlouva_soubor_cesta nvarchar(max) = null)
as
begin
	...
	begin
		set @query = 'set @bin_data = (select * FROM OPENROWSET(BULK  ''' + @smlouva_soubor_cesta + ''', SINGLE_BLOB) as varbinary)';
		exec sp_executesql @query, N'@bin_data varbinary(MAX) OUTPUT', @bin_data = @bin_data output
		insert into Hostování (Kontrakt_Id, Od, Do, Cena_Za_Sezónu, Smlouva, Hráč_Reg_Id, Členství_Id) VALUES (@kontrakt_id, @od, @do, @cena_za_sezonu, @bin_data, @hrac_reg_id, @clenstvi_id);
	end
	else if @smlouva_soubor_cesta is null
		insert into Hostování (Kontrakt_Id, Od, Do, Cena_Za_Sezónu, Hráč_Reg_Id, Členství_Id) VALUES (@kontrakt_id, @od, @do, @cena_za_sezonu, @hrac_reg_id, @clenstvi_id);
	else
...
as
begin
	insert into Hráč_Soupiska (Hráč_Reg_Id, Soupiska_Id, Číslo, Náhradník) VALUES (@reg_id, @soupiska_id, @cislo_dresu, @nahradnik)
end
GO
...
CREATE     procedure dbo.Přidej_Kontakt(@jmeno nvarchar(30), @prijmeni nvarchar(30), @email nvarchar(50) = null, @tel_cislo nvarchar(9) = null, @predvolba nvarchar(4) = null, @kontakt_id int output)
as
begin
...
CREATE procedure dbo.Přidej_Hráče(@reg_id nvarchar(7), @jmeno nvarchar(30), @prijmeni nvarchar(30), @pohlavi nvarchar(6), @d_nar date, @email nvarchar(50) = null, @tel_cislo nvarchar(9) = null, @predvolba nvarchar(4) = null)
as
begin
...
--Z existujícího kontaktu vytvoří rozhodčího
CREATE procedure dbo.Přidej_Rozhodčího(@jmeno nvarchar(30), @prijmeni nvarchar(30), @email nvarchar(50) = null, @tel_cislo nvarchar(9) = null, @predvolba nvarchar(4) = null, @rozhodci_id int output)
as
begin
	declare @tel_id int;
	BEGIN TRANSACTION T
...
--Nejde zlepšit, protože nejde vytvořit index na computed column Adresa(Využita). Pro vypočítání hodnoty tohoto sloupce se musí provést select do DB
...
--Vyžaduje 1x clustered index scan
--Nejde zlepšit, protože nejde vytvořit index na computed column Kontakt(Využit). Pro vypočítání hodnoty tohoto sloupce se musí provést select do DB
--Na takové sloupce nejde vytvořit index
...
--Vytvoří novou prázdnou soupisku, jejiž id vrací v druhém parametru
